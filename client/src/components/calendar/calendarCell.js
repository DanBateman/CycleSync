...
const flowChart = (num) => {
  return {
    background: `linear-gradient(180deg, white 50%, ${flow[num]})`,
  };
};

let ovulationStyles = {
  background: `linear-gradient(180deg, white 50%, #53c9ed)`,
};
let lutealStyles = {
  background: `linear-gradient(180deg, white 50%, #ede353)`,
};
let follicularStyles = {
  background: `linear-gradient(180deg, white 50%, #c953ed)`,
};
...
const CalendarCell = (props) => {
  ...
  const menstruation = useSelector((state) => state.calendar.menstruation);
  ...
  // Component variables
  let calcDays = calculate(lastMenstrualStart, avgCycle, avgMenses);
  let today = new Date();
  let nextPeriod = calcDays.next;
  let ovulationDay = calcDays.ov;
  ...
  const mensesToday = menstruation.filter((el) => {
    let date = new Date(el.date);
    return (
      date.getDate() === props.day.getDate() &&
      date.getMonth() === props.day.getMonth()
  ...
  const symptomOnClick = (e) => {
    e.stopPropagation();
    dispatch(setSelectedSymptom(props.day.getDate()));
    redirect();
    if (isMinWidth) {
      setOpen(true);
    }
  ...
  const dayOnClick = (e) => {
    e.stopPropagation();
    dispatch(setSelectedMenstruation(props.day.getDate()));
    redirect();
    if (isMinWidth) {
      setOpen(true);
    }
  ...
  return (
    <Box sx={{ ...(!(monthCheck || props.size == "small") && notMonth) }}>
      <Box
        sx={{
          ...styles,
          ...(mensesToday.length > 0 &&
            monthCheck &&
            flowChart(mensesToday[0].flow)),
          ...(props.size === "small" && smallCell),
        }}
        onClick={(e) => mensesToday.length > 0 && dayOnClick(e)}
      >
        <Box sx={{ display: "flex" }}>
          <Typography
            variant="h6"
          ...
          <Box sx={{ display: "flex", mr: 0.5 }}>
            ...
            {symptomsToday.length > 0 && (
              <CalendarTab variant="symptom" customClick={symptomOnClick} />
            )}
          </Box>
        </Box>
        <Box>
