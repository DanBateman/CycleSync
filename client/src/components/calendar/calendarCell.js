...
let menstrualStyles = {
  background: `linear-gradient(180deg, white 50%, ${flow[2]})`,
};
let ovulationStyles = {
        ...
        <Box sx={{ display: 'flex' }}>
          ...
          <Box sx={{ display: 'flex', mr: 0.5 }}>
            {activitiesToday.length > 0 && (
              <CalendarTab variant="activity" size={props.size} customClick={activityOnClick} />
            )}
            {mealsToday.length > 0 && (
              <CalendarTab variant="meal" size={props.size} customClick={mealOnClick} />
            )}
            {symptomsToday.length > 0 && <CalendarTab variant="symptom" />}
          </Box>
        </Box>
        <Box>
          {nextPeriodCheck && <FaTint style={{ marginLeft: '5px' }} color="#fa6b7e" />}
